...
await page.locator('#dsr-host .bplay').click();
await page.waitForTimeout(500);
const hlBefore = await page.evaluate(() => (window.CSS && window.CSS.highlights) ? Array.from(window.CSS.highlights.keys()).length : 0);
await page.locator('#dsr-host .ptitle').click();
await page.waitForTimeout(150);
assert(await page.locator('#dsr-host .fab.off').count() === 1, 'FAB greys out when reader disabled');
assert((await page.locator('#dsr-host .ptitle').getAttribute('aria-checked')) === 'false', 'header toggle reflects disabled state');
const hlAfter = await page.evaluate(() => (window.CSS && window.CSS.highlights) ? Array.from(window.CSS.highlights.keys()).length : 0);
assert(hlAfter === 0, 'highlights fully cleared when reader disabled (' + hlBefore + ' -> ' + hlAfter + ')');
...
await page.locator('#dsr-host .fab').click();
await page.waitForTimeout(150);
assert((await page.locator('#dsr-host .ptitle').getAttribute('aria-checked')) === 'false', 'disabled state persists across reload');
await page.locator('#dsr-host .ptitle').click();
await page.waitForTimeout(150);
assert(await page.locator('#dsr-host .fab.off').count() === 0, 'reader re-enabled from panel header');
